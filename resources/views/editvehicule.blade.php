                    ...
                    <div class="container" style="width:90%;height:300px">
                        ...
                        <div class="row">
                            <div class="container" style="width:400px;height:300px">
                                <br>
                                <div class="mb-1">
                                    <form action="{{route('updatevehicule')}}" method="post">
                                        @csrf
                                        <input type="hidden" name="id" value="{{$vehicule->id}}"> 
                                        <div class="mb-1">
                                            <label for="matricule" class="form-label"  >matricule</label>
                                            <input id="matricule" class="form-control mt-1 w-full" type="text" name="matricule" required autofocus value="{{$vehicule->matricule}}"  />
                                        </div>
                                        <div class="mb-1">
                                            <label for="prix" class="form-label"  >prix</label>
                                            <input id="prix" class="form-control mt-1 w-full" type="text" name="prix" required autofocus value="{{$vehicule->prix}}" />
                                        </div>
                                        <div class="mb-1">
                                            <label for="marque_id" class="form-label">marque</label>
                                            <select name="marque_id" id="marque_id" class="form-control">
                                                <option value="{{$vehicule->marque_id}}" selected>
